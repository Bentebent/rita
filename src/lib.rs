...
impl<'a> State<'a> {
    async fn new(window: &'a Window) -> State<'a> {
        ...
        let diffuse_bytes = include_bytes!(".\\dimmadome.jpg");
        let diffuse_texture = texture::Texture::from_bytes(&device, &queue, diffuse_bytes, Some("Dimmadome")).unwrap();
        ...
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("Screenspace Triangle"),
            source: wgpu::ShaderSource::Wgsl(include_str!("../data/shaders/tutorial_5.wgsl").into()),
        });
    ...
    fn input(&mut self, event: &WindowEvent) -> bool {
        match event {
            ...
            WindowEvent::KeyboardInput { event, .. } => matches!(
                event,
                KeyEvent {
...
pub async fn run() {
    ...
    let run = event_loop.run(move |event, control_flow| match event {
        Event::Resumed => {
            log::info!("Event loop resumed");
        }
    ...
    match run {
        Ok(_) => {
            log::info!("Event loop exited successfully");
        }
        Err(e) => {
            log::error!("Error: {:?}", e);
        }
